#!/usr/bin/env bash
# Publish the autoskill to ClawHub (OpenClaw registry).
#
# Prerequisites:
...
#   ./scripts/publish_to_clawhub.sh 1.0.0 "Initial release"

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_DIR="${ROOT}/autoskill"
VERSION="${1:-}"
CHANGELOG="${2:-}"
...
if [[ -z "${CLAWHUB_TOKEN:-}" ]]; then
  echo "error: set CLAWHUB_TOKEN (see: npx clawhub@latest login --no-browser --token <token>)" >&2
  exit 1
fi
...
echo "Published autoskill ${VERSION} to ClawHub."
